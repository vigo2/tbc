...
func (aa *AutoAttacks) ModifySwingTime(sim *Simulation, amount float64) {
	...
	if mhSwingTime := aa.MainhandSwingAt - sim.CurrentTime; mhSwingTime > 1 { // If its 1 we end up rounding down to 0 and causing a panic.
		aa.MainhandSwingAt = sim.CurrentTime + time.Duration(float64(mhSwingTime)/amount)
	}

	if aa.OH.SwingSpeed != 0 {
		if ohSwingTime := aa.OffhandSwingAt - sim.CurrentTime; ohSwingTime > 1 {
...
func (aa *AutoAttacks) NextAttackAt() time.Duration {
	...
	return MinDuration(aa.MainhandSwingAt, aa.OffhandSwingAt)
}
